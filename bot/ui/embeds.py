...
def panel_embed(snapshot: dict, mode: str, voice_snapshot: dict[str, object], music_snapshot: dict[str, object]) -> discord.Embed:
    preset = MODE_PRESETS.get(mode, MODE_PRESETS['normal'])
...
def status_embed(snapshot: dict, mode: str, asset_status: dict[str, str], model_name: str, voice_snapshot: dict[str, object], music_snapshot: dict[str, object]) -> discord.Embed:
    channels = snapshot.get('allowed_channel_ids', [])
    embed = shell('Status Wall', 'Runtime snapshot, AFK voice state, music queue, and asset health.', mode)
...
def help_embed(mode: str) -> discord.Embed:
    ...
    embed.add_field(name='/voice_join', value='Join the voice channel you are currently in.', inline=True)
...
def gallery_embed(mode: str, icon_names: list[str]) -> discord.Embed:
    ...
    embed = shell('Asset Gallery', 'Bundled visuals that back the command center, image analysis surface, AFK presence cards, and music controls.', mode)
