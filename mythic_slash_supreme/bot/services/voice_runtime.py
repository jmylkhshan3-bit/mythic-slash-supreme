...
class VoiceRuntimeManager:
    ...
    async def _process_utterance(self, guild_id: int, user_id: int, display_name: str, pcm: bytes) -> None:
        ...
        async with session.processing_lock:
            ...
            try:
                ...
                log.info('Voice transcript in guild %s from %s: %s', guild_id, display_name, text[:200])
                ...
                log.exception('Voice pipeline failed in guild %s', guild_id)
                await self._send_text_update(guild_id, f'Voice pipeline failed: {exc}')
    ...
    def _extract_voice_command(self, guild_id: int, user_id: int, text: str) -> str | None:
        ...
        configured = str(self.state_manager.get(guild_id).get('voice_wake_phrase', self.settings.activation_phrase)).lower().strip()
        normalized_text = self._normalize_command_text(text)
        phrases = [configured]
        if configured == 'hey m':
            phrases.extend(['hey em', 'hay m', 'hey him', 'هي ام', 'هاي ام', 'هيم'])
        phrases = [self._normalize_command_text(item) for item in phrases if item.strip()]
        ...
        for phrase in phrases:
            ...
            if index == -1:
                continue
            original_index = text.lower().find(configured)
            if original_index == -1:
                original_index = 0
            command = text[original_index + len(configured):].strip(' :,-')
            if command:
                return command
    ...
    async def _play_reply(self, session: GuildVoiceSession, vc: discord.VoiceClient, text: str, *, preferred_language: str | None = None) -> None:
        ...
        if not vc.is_playing():
            ...
            try:
                ...
                pass
            raise RuntimeError('Audio playback did not start. Check bot Speak permission and voice connection state.')
